...
C = struct('R', uint8(30), 'G', uint8(70),   'B',  uint8(35),  'TH', uint8(10)); % Origen 
 ...
 base_es  = struct('x1', 0, 'x2', 0, 'y1', 0, 'y2', 0, 'z1', 0, 'z2', 0);
 ...
 try
     ...
     avg = getHoughTransform(ba_A, 300, 50, 'MAX');
     base = avg;
     plot(base(:,1),base(:,2),'LineWidth',2,'Color','blue');
     ...
     % Third 'Eslabon' point coordinates
     base_es.x1 = x_dim(base(3), base(1));
     base_es.x2 = x_dim(base(4), base(2));

     base_es.y1 = y_dim(base(3), base(1));
     base_es.y2 = y_dim(base(4), base(2));

     base_es.z1 = z_dim(base(3), base(1));
     base_es.z2 = z_dim(base(4), base(2));
     
    a = [efec_fin.x2 efec_fin.y2 efec_fin.z2;
          efec_fin.x1 efec_fin.y1 efec_fin.z1];
    
    b = [seg_es.x2 seg_es.y2 seg_es.z2;
          seg_es.x1 seg_es.y1 seg_es.z1];
    
    c = [prim_es.x2 prim_es.y2 prim_es.z2;
          prim_es.x1 prim_es.y1 prim_es.z1];
      
    d = [base_es.x2 base_es.y2 base_es.z2;
          base_es.x1 base_es.y1 base_es.z1];
    
    getAnglesFromKinectPoints(a,b,c,d,origin)
